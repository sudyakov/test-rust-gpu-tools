...
__global__ void sortDescending(int *data, int n) {

  // получаем индекс текущей нити  
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  
  // сортируем данные с помощью библиотеки Thrust
  thrust::sort(thrust::device, data, data + n, thrust::greater<int>()); 
...
// KERNEL void cuda_blake3_hash(const uint32_t dimgrid, const uint32_t threads, GLOBAL uint32_t *cv, GLOBAL uint32_t *m, GLOBAL uint32_t *out)
//      {
// 	search<<<dimgrid, threads>>>(
// 		out,
// 		cv[0], cv[1], cv[2], cv[3], cv[4], cv[5], cv[6], cv[7],
...
KERNEL void hello(GLOBAL uint *result) {
    // Вывод на екран сообщения
    printf("blake3_test: Hello, World!\n");
}

extern "C" {

void cuda_sort_descending(int *data, int n) {
